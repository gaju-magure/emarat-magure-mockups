  ...
  return (
    <div className="h-full overflow-y-auto p-3 md:p-6">
      <div className="mb-4 md:mb-6">
        <h1 className="text-foreground mb-2 text-xl md:text-2xl">AI Governance</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-4 md:gap-6">
        ...
        <div className="bg-secondary backdrop-blur-md border border-border rounded-xl p-3 md:p-4">
          <h3 className="text-muted-foreground text-sm mb-3 px-2 hidden lg:block">Governance</h3>
        ...
        {/* Right Content */}
        <div className="bg-secondary backdrop-blur-md border border-border rounded-xl p-4 md:p-6">
          {activeSection === "registry" && (
            <div>
              <h2 className="text-foreground mb-4">Model Registry</h2>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <table className="w-full">
                  <thead className="bg-white/[0.03]">
                    <tr>
                      <th className="px-4 py-3 text-left text-sm text-muted-foreground">Model Name</th>
                      <th className="px-4 py-3 text-left text-sm text-muted-foreground">Version</th>
                      <th className="px-4 py-3 text-left text-sm text-muted-foreground">Owner</th>
                      <th className="px-4 py-3 text-left text-sm text-muted-foreground">Accuracy</th>
                      <th className="px-4 py-3 text-left text-sm text-muted-foreground">Status</th>
                      <th className="px-4 py-3 text-left text-sm text-muted-foreground">Last Updated</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {models.map((model, i) => (
                      <tr key={i} className="hover:bg-white/[0.03] transition-colors">
                        <td className="px-4 py-3 text-foreground">{model.name}</td>
                        <td className="px-4 py-3 text-foreground">{model.version}</td>
                        <td className="px-4 py-3 text-foreground">{model.owner}</td>
                        ...
                        <td className="px-4 py-3 text-muted-foreground text-sm">{model.lastUpdated}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {activeSection === "audit" && (
            <div>
              <h2 className="text-foreground mb-4">Audit Logs</h2>
              <p className="text-muted-foreground text-sm mb-6">
              ...
              <div className="space-y-3">
                {auditLogs.map((log, i) => (
                  <div
                    key={i}
                    className="bg-white/[0.03] border border-border rounded-lg p-4 hover:bg-secondary transition-all"
                  >
          ...
          {activeSection === "bias" && (
            <div>
              <h2 className="text-foreground mb-4">Bias & Risk Reports</h2>
              <p className="text-muted-foreground text-sm mb-6">
                  ...
                  >
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="text-foreground">{report.model}</h3>
                          <span
                            className={`inline-block px-2 py-1 rounded text-xs ${
                        ...
                        <p className="text-sm text-muted-foreground mb-2">{report.issue}</p>
          ...
          {activeSection === "policy" && (
            <div>
              <h2 className="text-foreground mb-4">Policy Manager</h2>
              <p className="text-muted-foreground text-sm mb-6">
              ...
              <div className="space-y-4">
                <div className="bg-white/[0.03] border border-border rounded-lg p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-foreground mb-1">AI Ethics Guidelines v2.0</h3>
                      <p className="text-sm text-muted-foreground">Updated: Sep 15, 2025</p>
                    </div>
                ...
                <div className="bg-white/[0.03] border border-border rounded-lg p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-foreground mb-1">Data Privacy Policy</h3>
                      <p className="text-sm text-muted-foreground">Updated: Aug 20, 2025</p>
                    </div>
                    <span className="inline-block px-3 py-1 rounded text-xs bg-green-500/20 text-green-400">
                      Active
                    </span>
          ...
          {activeSection === "data" && (
            <div>
              <h2 className="text-foreground mb-4">Data Catalog</h2>
              <p className="text-muted-foreground text-sm mb-6">
              ...
              <div className="space-y-4">
                {dataDomains.map((domain, i) => (
                  <div
                    key={i}
                    className="bg-white/[0.03] border border-border rounded-lg p-5"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-foreground mb-2">{domain.name}</h3>
                    ...
                    <div className="mb-3">
                      <p className="text-sm text-muted-foreground mb-2">Connected Models:</p>
                      <div className="flex flex-wrap gap-2">
                        {domain.connectedModels.map((model, j) => (
                          <span
                            key={j}
                            className="inline-block px-2 py-1 bg-secondary text-foreground rounded text-xs"
                          >
                            {model}
                          </span>
                        ))}
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">Owner: {domain.owner}</span>
                      <span className="text-muted-foreground">Last refresh: {domain.lastRefresh}</span>
                    </div>
                  </div>
                ))}
              </div>
